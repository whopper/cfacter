...
agents.each do |agent|
  ...
  on(agent, cfacter('--puppet', 'test_fact1', :environment => env)) do
    fail_test "Fact 1 not returned by cfacter --puppet test_fact1" unless
      stdout.include? 'test fact 1'
  end

  step "Agent: Verify test_fact2 from #{dir}/test_facts.rb"
  on(agent, cfacter('--puppet', 'test_fact2', :environment => env)) do
    fail_test "Fact 1 not returned by cfacter --puppet test_fact2" unless
